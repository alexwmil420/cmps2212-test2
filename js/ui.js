...
export function createUI(eventBus, gameService, rootEl) {
  ...
  function renderBoard(cards) {
    ...
    cards.forEach(card => {
      frag.appendChild(buildCardElement(card));
    });
  ...
  function markCardsMatched(firstId, secondId) {
    ...
    if (first) first.classList.add('is-matched');
    if (second) second.classList.add('is-matched');
  }
